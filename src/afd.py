...
class AFD:
    ...
    def validare(self, should_print:bool = True) -> bool:
        ...
        if len(stari_set) != len(self.Stari):
            este_valid = False
            print_invalid_rule(
                f"(1) Q trebuie sa contina stari distincte."
            )
            
        if len(set(self.Sigma)) != len(self.Sigma):
            este_valid = False
            print_invalid_rule(
                f"(2) Alfabetul sigma nu are simboluri distincte."
            )
            
        if not all(len(c) == 1 for c in self.Sigma):
            este_valid = False
            print_invalid_rule(
                f"(2) Alfabetul sigma nu are simboluri simple."
            )
        ...
        if self.StareInitiala not in self.Stari:
            este_valid = False
            print_invalid_rule(
                    f"(4) Starea initiala nu este din Q."
            )
            
        if not set(self.StariFinale).issubset(set(self.Stari)):
            este_valid = False
            print_invalid_rule(
                f"(5) Starile finale F nu sunt stari incluse in Q."
            )
